...
void loadAllDistributions()
{
  ...
  TFile fVertex("hVz_D0Sample.root");
   ...
   for (int iParticle = 0; iParticle < nParticles; ++iParticle)
   {
      ...
      std::cout << "Finished loading Dca: " <<  std::endl;
   }
   ...
   TFile fTpcPiPlus("Eff_PionPlus_embedding_FromHijing_1StopAll_16Oct13.root");
   TFile fTpcKMinus("Eff_KaonMinus_embedding_FromHijing_1StopAll_16Oct13.root");
...
bool matchHft(int const iParticleIndex, double const vz, int const cent, float const pt, float const eta)
{
   ...
   return gRandom->Rndm() < fMatch->Eval(pt);
}
