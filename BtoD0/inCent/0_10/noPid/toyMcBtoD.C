...
void toyMcBtoD(int npart = 10, TString output = "D0.toyMc.root", TString particleName = "D0", Int_t mWriteType=1, bool isCombinB = false)
{
    ...
    for (int ipart = 0; ipart < npart; ipart++)
    {
        ...
        if(mBackGround) {
            vector<float> vecRcPt;
            vector<float> vecRcEta;
            vector<float> vecRcPhi;
            vector<TVector3> vecPos;
            vector<int> vecType;
            ...
            for(int ik=0; ik<nKaon; ik++) {
                ...
                vecRcPt.push_back(rckFourMom.Perp());
                vecRcEta.push_back(rckFourMom.Eta());
                vecRcPhi.push_back(rckFourMom.Phi());
                vecPos.push_back(rckPos);
                vecType.push_back(1);  //0--pi, 1--k, 2--p
            }
            // pi smear
            ...
            for(int ipi=0; ipi<nPion; ipi++) {
                ...
                vecRcPt.push_back(rcpiFourMom.Perp());
                vecRcEta.push_back(rcpiFourMom.Eta());
                vecRcPhi.push_back(rcpiFourMom.Phi());
                vecPos.push_back(rcpiPos);
                vecType.push_back(0);  //0--pi, 1--k, 2--p
            }
            // p smear
            TVector3 pPos(0,0,0);
            ...
            for(int ip=0; ip<nProton; ip++) {
                ...
                rcpPos = smearPosData(2, 0, 7, rcpFourMom, pPos); //1--k, 0--pi
                vecRcPt.push_back(rcpFourMom.Perp());
                vecRcEta.push_back(rcpFourMom.Eta());
                vecRcPhi.push_back(rcpFourMom.Phi());
                vecPos.push_back(rcpPos);
                vecType.push_back(2);  //0--pi, 1--k, 2--p
            }
            
            //reconstruction
            for(int ik=0; ik<nKaon+nPion+nProton; ik++) {
                TVector3 vertex(0,0,0);
                float kpt = vecRcPt[ik];
                float keta = vecRcEta[ik];
                float kphi = vecRcPhi[ik];
                if(kpt<0.6) continue;
                ...
                if(vecType[ik]==0) {
                    ...
                    ktof = 1;//(kpt<=1.6 ? fTofPi->Eval(kpt) : 1.0);
                    ktpc = fTpcPi->Eval(kpt);
                }
                else if(vecType[ik]==1) {
                    kPtWg = hkPtWg->GetBinContent(hkPtWg->FindBin(kpt));
                    ktof = 1;//(kpt<=1.6 ? fTofK->Eval(kpt) : 1.0);
                    ktpc = fTpcK->Eval(kpt);
                }
                else if(vecType[ik]==2) {
                    kPtWg = hpPtWg->GetBinContent(hpPtWg->FindBin(kpt));
                    ktof = 1;//(kpt<=1.6 ? fTofP->Eval(kpt) : 1.0);
                    ktpc = fTpcP->Eval(kpt);
                }
                else {
                    continue;
                }
                for(int ipi=0; ipi<nKaon+nPion+nProton; ipi++) {
                    if(ipi==ik) continue;
                    float pipt = vecRcPt[ipi];
                    float pieta = vecRcEta[ipi];
                    float piphi = vecRcPhi[ipi];
                    ...
                    if(vecType[ipi]==0) {
                        ...
                        pitof = 1;//(pipt<=1.6 ? fTofPi->Eval(pipt) : 1.0);
                        pitpc = fTpcPi->Eval(pipt);
                    }
                    else if(vecType[ipi]==1) {
                        piPtWg = hkPtWg->GetBinContent(hkPtWg->FindBin(pipt));
                        pitof = 1;//(pipt<=1.6 ? fTofK->Eval(pipt) : 1.0);
                        pitpc = fTpcK->Eval(pipt);
                    }
                    else if(vecType[ipi]==2) {
                        piPtWg = hpPtWg->GetBinContent(hpPtWg->FindBin(pipt));
                        pitof = 1;//(pipt<=1.6 ? fTofP->Eval(pipt) : 1.0);
            ...
            vecRcPt.clear(); vecRcEta.clear(); vecRcPhi.clear(); vecPos.clear(); vecType.clear();
        }//end background
...
void bookObjects()
{
    ...
    h3PiKPNumber = (TH3I*)fDaughters.Get("h3PiKPNumber0_10");
    ...
    fTofK->SetParameters(-0.00377627, 0.198926, 0.00805162, -9.01765, -0.653181, -0.523133, 0.58466);
    fTofPi = new TF1("fTofPi","[0]/(pow(x-[1],2)+[2])-[4]/(exp(x-[3])+[5])+[6]",0.2,3);
    fTofPi->SetParameters(-0.0743809, -2.89051, -8.97966, -0.199471, 2.26781e-07, -1.69786, 0.602073);
    fTofP = new TF1("fTofP","[0]/(pow(x-[1],2)+[2])-[4]/(exp(x-[3])+[5])+[6]",0.2,3);
    fTofP->SetParameters(-0.0110193, 0.257745, 0.0203688, -0.196571, -4.60251e-05, -1.70964, 0.596767);
    
    cout << "Loading D0/B pT weight ..." << endl;
    TFile fD0PtWeight("D0Pt_Wg.root"); //0-12GeV
    hD0PtWgFonll = (TH1F*)fD0PtWeight.Get("hD0Pt_wg_0_10");
    hD0PtWgFonll->SetDirectory(0);
    hBPtWgFonll = (TH1F*)fD0PtWeight.Get("hBPt_wg_0_10");
