...
//input 4--background from hijing sample
...
void toyMcBtoD(int npart = 10, TString output = "D0.toyMc.root", TString particleName = "D0", Int_t mWriteType=1, bool isCombinB = false)
{
    ...
    for (int ipart = 0; ipart < npart; ipart++)
    {
        ...
        if(mBackGround) {
            ...
            //reconstruction
            for(int ik=0; ik<nKaon+nPion; ik++) {
                ...
                for(int ipi=ik; ipi<nKaon+nPion; ipi++) {
                    ...
                    float piPtWg = 1.;
                    if(ipi<nKaon) piPtWg = hkPtWg->GetBinContent(hkPtWg->FindBin(pipt));
                    else piPtWg = hpiPtWg->GetBinContent(hpiPtWg->FindBin(pipt));
            ...
            vecRcPt.clear(); vecRcEta.clear(); vecRcPhi.clear(); vecPos.clear();
        }//end background
